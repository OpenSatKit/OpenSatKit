...
#
# $Date: 2008/10/27 19:56:11EDT $
# $Revision: 1.1 $
# $Log: utf_sbsim.pm  $
# Revision 1.1 2008/10/27 19:56:11EDT sstrege 
# Initial revision
...

{
    ...
    $result = GetOptions("endian=i" => \$endian);
}
    ...
    for ($i=0; $i < @tlm_packet_table; $i++) {
        ...
        if ($tlm_packet_table[$i]{NAME} eq $name) {
            ...
            if ($tlm_packet_table[$i]{SEC}) {
                ...
                # time is now stored as 4 16 bit words so I split it up into seperate writes to make the
                # bytes come out in the right order
                put_word(\$packet, 6,  ($seconds & 0xFFFF0000) >> 16);    #time seconds
                put_word(\$packet, 8,  ($seconds & 0x0000FFFF));          #time seconds
                put_word(\$packet, 10, ($subseconds & 0xFFFF0000) >> 16); #time subseconds
                put_word(\$packet, 12, ($subseconds & 0x0000FFFF));       #time subseconds
                $byte_offset += 8;
            }
...
sub put_byte
{
    ...
    substr($$buffer_ref, $offset * 2, 2) = unpack('H2', reverse(pack("C", $data)));
}
...
sub put_word
{
    ...
    if ($endian == 0) {
        substr($$buffer_ref, $offset * 2, 4) = swap_bytes(unpack('H4', reverse(pack("S", $data))), 2); #le
    } else {
...
sub put_dword
{
    ...
    if ($endian == 0) {
        substr($$buffer_ref, $offset * 2, 8) = swap_bytes(swap_words(unpack('H8', reverse(pack("L", $data))), 4), 4); #le
    } else {
...
sub put_float
{
    ...
    if ($endian == 0) {
        substr($$buffer_ref, $offset * 2, 8) = swap_bytes(swap_words(unpack('H8', reverse(pack("f", $data))), 4), 4); #le
    } else {
...
sub put_double
{
    ...
    if ($endian == 0) {
        substr($$buffer_ref, $offset * 2, 16) = swap_bytes(swap_words(unpack('H16', reverse(pack("d", $data))), 8), 8); #le
    } else {
...
sub get_byte
{
    ...
    return hex(substr($$buffer_ref, $offset * 2, 2));
}
...
sub get_word
{
    ...
    if ($endian == 0) {
        return hex(swap_bytes(substr($$buffer_ref, $offset * 2, 4), 2));
    } else {
        return hex(substr($$buffer_ref, $offset * 2, 4));
    }
}
...
sub get_dword
{
    ...
    return hex(swap_words(substr($$buffer_ref, $offset * 2, 8), 4));
}
...
sub get_float
{
    ...
    return unpack('f', reverse(pack("H8", swap_words(substr($$buffer_ref, $offset * 2, 8), 4))));
}
...
sub get_double
{
    ...
    return unpack('d', reverse(pack("H16", swap_words(substr($$buffer_ref, $offset * 2, 16), 8))));
}
