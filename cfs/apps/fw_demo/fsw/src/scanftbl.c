...
**   2. The load and dump functions use call back functions to investigate
...
boolean SCANFTBL_LoadCmd(TBLMGR_Tbl *Tbl, uint8 LoadType, const char* Filename)
{
   ...
   OS_printf("SCANFTBL_LoadCmd() Entry\n");
...
void ParseLine(char *FileLineBuff)
{
   ...
   if (ScanfCnt == 4)
   {
      ...
      {        
         ...
         /* OS_printf("SCANFTBL-ParseLine: %d, %d, %d, %d\n",Index, Data1, Data2, Data3); */
      }
      else
      {
         ScanfTbl->AttrErrCnt++;     
   ...
   {
      ScanfTbl->AttrErrCnt++;
      CFE_EVS_SendEvent(SCANFTBL_LOAD_LINE_ERR_EID, CFE_EVS_ERROR, "Load file line %d error, invalid number of elements %d. Shoudl be 4.",
