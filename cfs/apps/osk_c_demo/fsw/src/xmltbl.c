...
boolean XMLTBL_LoadCmd(TBLMGR_Tbl *Tbl, uint8 LoadType, const char* Filename)
{
   ...
   CFE_EVS_SendEvent(OSK_C_DEMO_INIT_DEBUG_EID, OSK_C_DEMO_INIT_EVS_TYPE, "XMLTBL_LoadCmd() Entry\n");
   ...
   */
       ...
       XmlTbl->AttrErrCnt == 0)
   {
      ...
      {
         CFE_EVS_SendEvent(OSK_C_DEMO_INIT_DEBUG_EID, OSK_C_DEMO_INIT_EVS_TYPE, "XMLTBL_LoadCmd() Before LoadTblFunc\n");
         XmlTbl->LastLoadStatus = ((XmlTbl->LoadTblFunc)(&(XmlTbl->Tbl)) == TRUE) ? TBLMGR_STATUS_VALID : TBLMGR_STATUS_INVALID;
         CFE_EVS_SendEvent(OSK_C_DEMO_INIT_DEBUG_EID, OSK_C_DEMO_INIT_EVS_TYPE, "XMLTBL_LoadCmd() After LoadTblFunc\n");
