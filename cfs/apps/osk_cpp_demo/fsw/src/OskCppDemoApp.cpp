/* 
** Purpose: C++ demo app using the OSK C++ App Framework
**
** Notes:
**   None
**
...
enum EID {
   ...
   EID_APP_INIT = AppCfg::OSK_CPP_DEMO_APP_BASE_EID,
   EID_APP_EXIT,
   EID_INVALID_CMD_MID
...
/******************************************************************************
** Function: OskCppDemo_AppMain
**
** Notes:
**   1. C function used as cFE entry point.
*/

static OskCppDemoApp g_osk_cpp_demo_app (AppCfg::MAJOR_VER, AppCfg::MINOR_VER, AppCfg::LOCAL_REV);


// Naming convention would dictate OSK_CPP_DEMO_AppMain but the name gets too long
extern "C" void OSK_CPP_DEMO_Main(void);
void OSK_CPP_DEMO_Main(void) {
   
   g_osk_cpp_demo_app.Execute();

}

OskCppDemoApp::OskCppDemoApp(int major_ver, int minor_ver, int local_rev) : App(major_ver, minor_ver, local_rev) , 
   m_ex_app_obj(AppCfg::OSK_CPP_DEMO_APP_BASE_EID) {
...
OskCppDemoApp::~OskCppDemoApp() {}

void OskCppDemoApp::Execute() {
   
   int32  status        = CFE_SEVERITY_ERROR;
   uint32 es_app_status = CFE_ES_APP_ERROR;


   CFE_ES_PerfLogEntry(OSK_CPP_DEMO_MAIN_PERF_ID);
   ...
   */
   while (CFE_ES_RunLoop(&es_app_status)) {
      ...
      */
	   ...
	   CFE_ES_PerfLogExit(OSK_CPP_DEMO_MAIN_PERF_ID);
      OS_TaskDelay(OSK_CPP_DEMO_RUNLOOP_DELAY);
      CFE_ES_PerfLogEntry(OSK_CPP_DEMO_MAIN_PERF_ID);
...
boolean OskCppDemoApp::ResetState() {
...
int32 OskCppDemoApp::Init() {
   ...
   */
   ...
   CFE_SB_CreatePipe(&m_cmd_pipe, OSK_CPP_DEMO_CMD_PIPE_DEPTH, OSK_CPP_DEMO_CMD_PIPE_NAME);
   CFE_SB_Subscribe(OSK_CPP_DEMO_CMD_MID, m_cmd_pipe);
   CFE_SB_Subscribe(OSK_CPP_DEMO_SEND_HK_MID, m_cmd_pipe);
   ...
   */
   ...
   CFE_SB_InitMsg(&m_hk_pkt, OSK_CPP_DEMO_APP_TLM_HK_MID, OSK_CPP_DEMO_APP_TLM_HK_LEN, TRUE);
   ...
   */
   status = CFE_EVS_SendEvent(EID_APP_INIT,
                              CFE_EVS_INFORMATION,
                              "OSK C++ Demo App Initialized. Version %d.%d.%d",
...
**
*/
void OskCppDemoApp::SendHousekeepingPkt() {
...
**
*/
void OskCppDemoApp::ProcessCommands() {
   ...
   if (status == CFE_SUCCESS) {
      ...
      switch (msg_id) {
         ...
         case OSK_CPP_DEMO_CMD_MID:
            m_cmd_mgr.ProcessCmd(cmd_msg_ptr);
            break;

         case OSK_CPP_DEMO_SEND_HK_MID:
