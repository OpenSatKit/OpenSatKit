...
enum EID {
   ...
   EID_DEMO_CMD = AppCfg::EX_APP_OBJ_BASE_EID,
   EID_INVALID_TBL_ID,
...
ExAppObj::ExAppObj(uint16 event_id_base) : AppObj(event_id_base),
   ...
   m_ex_jsmn_tbl(EXTBL_DEF_LOAD_FILE),
   m_ex_rapid_tbl(EXTBL_DEF_LOAD_FILE),
...
uint8 ExAppObj::RegisterCmds (oskfw::CmdMgr& cmd_mgr) {
   ...
   cmd_mgr.RegisterCmd(m_demo_cmd, AppCfg::EX_APP_OBJ_DEMO_CMD_FC);
...
uint8 ExAppObj::RegisterTbls (oskfw::TblMgr& tbl_mgr) {
   ...
   //m_ex_cfe_tbl.Register(ValidateExCfeTbl);
   m_ex_cfe_tbl.Register(ValidateExCfeTbl,"/cf/ExCfeTbl.tbl");
...
ExAppObj::DemoCmd::DemoCmd(ExAppObj* ex_app_obj, uint16 data_len) : oskfw::CmdMsg(data_len) , m_ex_app_obj(ex_app_obj) {}

boolean ExAppObj::DemoCmd::Process(const CFE_SB_MsgPtr_t msg_ptr) {
   ...
   const ExAppObj_DemoCmdMsg *cmd_msg = (const ExAppObj_DemoCmdMsg *) msg_ptr;
   ...
   if (valid_table) {
      ...
      if (valid_entry) {
         ...
         CFE_EVS_SendEvent (EID_DEMO_CMD, CFE_EVS_INFORMATION,
