...
def go(screen, method)
	...
	if (method == "enable_telemetry")
		enable_telemetry()
	elsif (method == "send_parameters")
...
def send_parameters(screen)
	...
	cmd("CFS BM_SENDHK")
  	...
  	parameter_cmd_str = "CFS BM_SETPARAMS with"
...
def run_benchmarks(screen)
	...
	cmd("CFS BM_SENDHK")
  	wait_packet("CFS", "CFE_BM_HKTLMPKT", 1, 5)
	wait_check_expression("tlm('CFS CFE_BM_HKTLMPKT BM_RUNNINGBENCHMARKS') == 0", 1000)
	cmd("CFS BM_RUNALL")
...
def run_benchmarks_multi(screen)
	...
	if (text_widget.text == "")
		benchmark_dir = "#{Cosmos::USERPATH}/Benchmark data/"
	...
	for i in 0..(num_datapoints - 1)
	    ...
	    cmd("CFS BM_SENDHK")
  		wait_packet("CFS", "CFE_BM_HKTLMPKT", 1, 5)
		cmd("CFS BM_RUNALL")
		wait_check_expression("tlm('CFS CFE_BM_HKTLMPKT BM_RUNNINGBENCHMARKS') == 1", 1000)
		wait_check_expression("tlm('CFS CFE_BM_HKTLMPKT BM_RUNNINGBENCHMARKS') == 0", 1000)
		
		benchmark_data.each_pair {|key, value|
			value[i] = tlm("CFS CFE_BM_HKTLMPKT #{key.upcase}")
		}
