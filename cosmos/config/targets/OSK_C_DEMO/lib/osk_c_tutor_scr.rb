...
def osk_c_tutor_cmd(screen, cmd)
   ...
   case cmd 
   when "REF_TUTORIAL"
      tutor_file = File.join(tutor_doc_dir,CFS_EDU_APP_TUTOR_GUIDE_FILE)
      Osk::System.display_pdf(tutor_file)
   
   when "REF_VIDEO"
      prompt(Osk::MSG_TBD_FEATURE + "\n")   
      #Cosmos.open_in_web_browser(CFS_EDU_YOUTUBE_APP_TUTOR_INTRO)
   ...
   when "TUTORIAL_START_RESUME"
      ...
      base_json_file = File.join(lesson_dir,OskCDemo::LESSON_JSON_BASE_FILE)
      ...
      display("OSK_C_DEMO OSK_C_LESSON_SCR")
   
   when "TUTORIAL_RESTART"
...
def osk_c_tutor_start
   ...
   base_json_file = File.join(tutor_app_dir,OskCDemo::TUTOR_JSON_BASE_FILE)
   user_json_file = File.join(tutor_app_dir,OskCDemo::TUTOR_JSON_USER_FILE)
   ...
   if (File.exists?(user_json_file))
      ...
      tutor_json = File.read(user_json_file)
      ...
      tutor_json = File.read(base_json_file)
      tutor_info = JSON.parse(tutor_json)
      ...
      lesson_1_json_file = File.join(tutor_app_dir,'lesson','1',OskCDemo::LESSON_JSON_BASE_FILE)            
   ...
   (1..100).each do |lesson|
      ...
      begin
         lesson_json_file = File.join(lesson_dir,OskCDemo::LESSON_JSON_BASE_FILE)
...
def osk_c_tutor_delete_user_files
   ...
   tutor_app_dir  = Osk::fsw_app_dir(OskCDemo::TUTOR_APP_NAME)
   user_tutor_json_file = File.join(tutor_app_dir,OskCDemo::TUTOR_JSON_USER_FILE)
   File.delete(user_tutor_json_file) if File.exist?(user_tutor_json_file)
   ...
   (1..100).each do |lesson|
      ...
      user_lesson_json_file = File.join(lesson_dir,OskCDemo::LESSON_JSON_USER_FILE)
...
def osk_c_tutor_create_scr(user_state, tutor_info, lesson_titles)
   ...
   VERTICALBOX \"Tutorial\"
      ...
      MATRIXBYCOLUMNS 4 5 5
         BUTTON 'Start/Resume' 'osk_c_tutor_cmd(self,\"TUTORIAL_START_RESUME\")'
         NAMED_WIDGET lesson COMBOBOX #{lesson_list}
         BUTTON 'Restart'      'osk_c_tutor_cmd(self,\"TUTORIAL_RESTART\")'
         LABEL  '        '
      END
   ...
   scr_file = Osk::cfg_target_dir_file("OSK_C_DEMO","screens","#{OskCDemo::TUTOR_APP_NAME}_scr.txt")
