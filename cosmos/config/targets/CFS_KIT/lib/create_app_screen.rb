...
TEMPLATE_DIR = {"cFE Sample App" => "cfe-sample_app", "cFE Sample Lib" => "cfe-sample_lib", "OSK Framework App" => "osk-fw_app"}
...
def create_app_display_template_info(screen)
   ...
   template_info = AppTemplate.get_descr(File.join(Osk::CREATE_APP_DIR,'templates',TEMPLATE_DIR[template_sel]))

   create_app_create_template_info_screen(template_sel, template_info)

   display("CFS_KIT #{Osk::TEMPLATE_INFO_SCR_FILE.split('.').first.upcase}",50,50)
