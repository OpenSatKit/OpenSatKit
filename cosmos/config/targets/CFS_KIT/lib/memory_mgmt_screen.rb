...
def memory_mgmt_send_cmd(screen, cmd)
    ...
    cmd("MM LOOKUP_SYMBOL with SYMBOL_NAME #{symbol_name}")
	elsif (cmd == "PEEK_MEM")
    ...
    cmd("MM PEEK_MEM with DATA_SIZE #{data_size}, MEM_TYPE #{mem_type}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
	elsif (cmd == "POKE_MEM")
    mem_type = ask_string("Enter memory type (0=None, 1=RAM, 2=EEPROM")
    data_size = ask_string("Enter data size in bits (8, 16, 32)")
    addr_symbol_name = ask_string("Enter symbol to be used as base address or empty string if no symbol")
    ...
    cmd("MM POKE_MEM with DATA_SIZE #{data_size}, MEM_TYPE #{mem_type}, DATA #{data}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
	elsif (cmd == "DUMP_TO_EVENT")
    mem_type = ask_string("Enter memory type (0=None, 1=RAM, 2=EEPROM")
    num_bytes = ask_string("Enter number of bytes to dump (Max 15)")
    addr_symbol_name = ask_string("Enter symbol to be used as base address or empty string if no symbol")
    addr_offset = ask_string("Enter offset from symbol or absolute address if no symbol specified")
    cmd("MM DUMP_IN_EVENT with MEM_TYPE #{mem_type}, NUM_BYTES #{num_bytes}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
    ...
    addr_symbol_name = ask_string("Enter symbol to be used as base address or empty string if no symbol")
    addr_offset = ask_string("Enter offset from symbol or absolute address if no symbol specified")
    cmd("MM FILL_MEM with MEM_TYPE #{mem_type}, NUM_BYTES #{num_bytes}, FILL_PATTERN #{fill_pattern}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
	elsif (cmd == "LOAD_FROM_FILE")
    #TODO - Add file put
    mem_file_name = ask_string("Enter full FSW path/filename of memory load file")
    cmd("MM LOAD_MEM_FROM_FILE with FILE_NAME #{mem_file_name}")
    ...
    table_mask  = ask_string("Enter table bit mask with bit set to 1 to start a table")
    cmd("MD START_DWELL with TABLE_MASK #{table_mask}")  
  elsif (cmd == "STOP_DWELL")
    table_mask  = ask_string("Enter table bit mask with bit set to 1 to stop a table")
    cmd("MD STOP_DWELL with TABLE_MASK #{table_mask}")  
