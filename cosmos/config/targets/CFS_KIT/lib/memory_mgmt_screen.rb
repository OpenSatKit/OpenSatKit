...
def memory_mgmt_send_cmd(screen, cmd)
	...
	if (cmd == "LOOKUP_SYMBOL")
      ...
      Osk::flight.mm.send_cmd("LOOKUP_SYMBOL with SYMBOL_NAME #{symbol_name}")
	elsif (cmd == "PEEK_MEM")
      ...
      Osk::flight.mm.send_cmd("PEEK_MEM with DATA_SIZE #{data_size}, MEM_TYPE #{mem_type}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
	elsif (cmd == "POKE_MEM")
      mem_type = ask_string("Enter memory type (0=None, 1=RAM, 2=EEPROM")
      data_size = ask_string("Enter data size in bits (8, 16, 32)")
      addr_symbol_name = ask_string("Enter symbol to be used as base address or empty string if no symbol")
      ...
      Osk::flight.mm.send_cmd("POKE_MEM with DATA_SIZE #{data_size}, MEM_TYPE #{mem_type}, DATA #{data}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
	elsif (cmd == "DUMP_TO_EVENT")
      mem_type = ask_string("Enter memory type (0=None, 1=RAM, 2=EEPROM")
      num_bytes = ask_string("Enter number of bytes to dump (Max 15)")
      addr_symbol_name = ask_string("Enter symbol to be used as base address or empty string if no symbol")
      addr_offset = ask_string("Enter offset from symbol or absolute address if no symbol specified")
      Osk::flight.mm.send_cmd("DUMP_IN_EVENT with MEM_TYPE #{mem_type}, NUM_BYTES #{num_bytes}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
      ...
      addr_symbol_name = ask_string("Enter symbol to be used as base address or empty string if no symbol")
      addr_offset = ask_string("Enter offset from symbol or absolute address if no symbol specified")
      Osk::flight.mm.send_cmd("FILL_MEM with MEM_TYPE #{mem_type}, NUM_BYTES #{num_bytes}, FILL_PATTERN #{fill_pattern}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
      ...
      Osk::flight.mm.send_cmd("LOAD_MEM_FROM_FILE with FILENAME #{mem_path_filename}")
      ...
      cmd("MM DUMP_MEM_TO_FILE with MEM_TYPE #{mem_type}, NUM_BYTES #{num_bytes}, ADDR_OFFSET #{addr_offset}, ADDR_SYMBOL_NAME #{addr_symbol_name}, FILE_NAME #{mem_file_name}")
      ...
      Osk::flight.md.send_cmd("START_DWELL with TABLE_MASK #{table_mask}")  
   elsif (cmd == "STOP_DWELL")
      table_mask  = ask_string("Enter table ID bit mask with bit set to 1 to stop a dwellctable")
      Osk::flight.md.send_cmd("STOP_DWELL with TABLE_MASK #{table_mask}")  
      ...
      addr_symbol_name = ask_string("Enter symbol to be used as base address or empty string if no symbol")
      addr_offset = ask_string("Enter offset from symbol or absolute address if no symbol specified")
      Osk::flight.md.send_cmd("JAM_DWELL with TABLE_ID #{table_id}, ENTRY_ID #{entry_id}, FIELD_LEN #{field_len}, ADDR_OFFSET #{addr_offset}, DELAY #{interval}, ADDR_SYMBOL_NAME #{addr_symbol_name}")
