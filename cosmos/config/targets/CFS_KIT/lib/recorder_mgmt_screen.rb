...
def recorder_mgmt_send_cmd(screen, cmd)
	...
	if (cmd == "APP_STATE")
      ...
      Osk::flight.ds.send_cmd("SET_APP_STATE with APP_STATE #{app_state}")
      ...
      Osk::flight.ds.send_cmd("SET_FILE_STATE with FILE_TBL_IDX #{file_tbl_idx}, FILE_STATE #{file_state}")
	elsif (cmd == "FILE_COUNT")
      file_tbl_idx = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
      sequence_cnt = ask_string("Enter sequence count portion fo filename","0")
      Osk::flight.ds.send_cmd("SET_FILE_COUNT with FILE_TBL_IDX #{file_tbl_idx}, SEQUENCE_CNT #{sequence_cnt}")
	elsif (cmd == "FILE_TYPE")
      file_tbl_idx  = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
      filename_type = ask_string("Enter sequence count portion of filename","0")
      Osk::flight.ds.send_cmd("SET_FILE_TYPE with FILE_TBL_IDX #{file_tbl_idx}, FILENAME_TYPE #{filename_type}")
	elsif (cmd == "FILE_PATH")
      file_tbl_idx = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
      path_name    = ask_string("Enter file path","/cf")
      Osk::flight.ds.send_cmd("SET_FILE_PATH_NAME with FILE_TBL_IDX #{file_tbl_idx}, PATH_NAME #{path_name}")
	elsif (cmd == "FILE_BASE")
      file_tbl_idx = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
      base_name    = ask_string("Enter file base name","")
      Osk::flight.ds.send_cmd("SET_FILE_BASE_NAME with FILE_TBL_IDX #{file_tbl_idx}, BASE_NAME #{base_name}")
	elsif (cmd == "FILE_EXTENSION")
      file_tbl_idx = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
      extension    = ask_string("Enter file extension","")
      Osk::flight.ds.send_cmd("SET_FILE_EXT with FILE_TBL_IDX #{file_tbl_idx}, EXTENSION #{extension}")
	elsif (cmd == "FILE_SIZE")
      file_tbl_idx  = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
      max_file_size = ask_string("Enter the maximum file size (bytes)","")
      Osk::flight.ds.send_cmd("SET_FILE_MAX_SIZE with FILE_TBL_IDX #{file_tbl_idx}, MAX_FILE_SIZE #{max_file_size}")
	elsif (cmd == "FILE_AGE")
      file_tbl_idx = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
      max_file_age = ask_string("Enter the maximum file age (secs)","")
      Osk::flight.ds.send_cmd("SET_FILE_MAX_AGE with FILE_TBL_IDX #{file_tbl_idx}, MAX_FILE_AGE #{max_file_age}")
	elsif (cmd == "FILE_CLOSE")
      close = message_box("Close one or all files?", 'One', 'All')
      if (close == "One")
         file_tbl_idx = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
         Osk::flight.ds.send_cmd("CLOSE_FILE with FILE_TBL_IDX #{file_tbl_idx}")
      elsif (close == "All")
      ...
      file_tbl_idx     = ask_string("Enter destination file index 0..#{DEST_FILE_TBL_MAX_IDX}","0")
      Osk::flight.ds.send_cmd("SET_FILTER_FILE with MSG_ID #{msg_id}, FILTER_PARAM_IDX #{filter_param_idx}, FILE_TBL_IDX #{file_tbl_idx}")
   elsif (cmd == "FILTER_ADD_MSG")
      msg_id = ask_string("Message ID of existing entry in Packet Filter Table","0")
      Osk::flight.ds.send_cmd("ADD_FILTER_MID with MSG_ID #{msg_id}")  
      ...
      Osk::flight.ds.send_cmd("SET_FILTER_PARAM with MSG_ID #{msg_id}, FILTER_PARAM_IDX #{filter_param_idx}, ALGORITHM_N #{algorithm_n}, ALGORITHM_X #{algorithm_x}, ALGORITHM_O #{algorithm_o}")
   elsif (cmd == "FILTER_TYPE")
      msg_id           = ask_string("Message ID of existing entry in Packet Filter Table","0")
      filter_param_idx = ask_string("Enter index into message's array of filters: 0..#{FILTER_PARAM_MAX_IDX}","0")
      filter_type      = ask_string("Enter filter type: 1=Sequence Count, 2=Time","1")
      Osk::flight.ds.send_cmd("SET_FILTER_TYPE with MSG_ID #{msg_id}, FILTER_PARAM_IDX #{filter_param_idx}, FILTER_TYPE #{filter_type}")
	elsif (cmd == "GET_FILE")
