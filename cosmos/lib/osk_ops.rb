...
module Ops
   ...
   # Default gnd_path shold be overriden for tables that have a separate file server directory
   def self.get_flt_file_prompt(gnd_path = GND_SRV_DIR)
      flt_path_filename = ask_string("Enter full FSW /path/filename.")
   ...
   #
   def self.send_flt_bin_file_cmd(app_name, cmd_str, tbl_mgr_def_filename)
      flt_path_filename = "#{FLT_SRV_DIR}/#{TMP_BIN_FILE}"
      if (Osk::flight.app[app_name].send_cmd("#{cmd_str} FILENAME #{flt_path_filename}"))
         get_file = combo_box("Transfer file from flight to ground and display it?", 'Yes','No')
         if (get_file == "Yes")
            get_flt_bin_file(flt_path_filename, REL_SRV_DIR, TMP_BIN_FILE, tbl_mgr_def_filename)
         end
      else
         raise_exception "osk_ops.send_flt_bin_file_cmd() - File Manager(FM) 'dir to file' command failed"
      end
   ...
   def self.create_tbl_mgr_scr(rel_bin_file_path, bin_filename, tbl_def_filename)
      ...
      #
      # Temporary screen automatically created.
      #
      # License:
      #   Written by David McComas, licensed under the copyleft GNU General Public
      #   License (GPL).
